...
public class HappyAdapter extends BaseAdapter {
    ...
    public HappyAdapter(Context context) {
    ...
    @Override
    public int getCount() {
        return data != null ? data.getResult().getRadio().getResult().size() : 0;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        HappyViewHolder holder = null;
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.item_recommend_newcd, parent, false);
            holder = new HappyViewHolder(convertView);
            convertView.setTag(holder);
        } else {
            holder = (HappyViewHolder) convertView.getTag();

        }
        holder.titleTv.setText(data.getResult().getRadio().getResult().get(position).getTitle());
        holder.nameTv.setVisibility(View.GONE);
        Picasso.with(context).load(data.getResult().getRadio().getResult().get(position).getPic())
                .resize(230, 230).into(holder.iconIv);
        return convertView;
    }

    class HappyViewHolder {
        ImageView iconIv;
